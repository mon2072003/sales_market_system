...
//    int h;
    int remainders;
    int prints;
    purchases.v a=new purchases.v();
    purchases.v1 a1=new purchases.v1();
    int bill=0;
...
public  purchases() { 
this.setName("مشتريات");
this.setLayout(null);
Color col=new Color(200,200,100);
this.setBackground(col);
/*****************************icon*******************************************/
ImageIcon icon =new ImageIcon(this.getClass().getResource("/product.png"));
JLabel im=new JLabel(icon);
im.setBounds(20, 40, 265, 265);
this.add(im);
/****************************buttons*****************************************/
...
@Override
    public void actionPerformed(ActionEvent e) {
        if (e.getSource()==b1){
            try {
          c= DriverManager.getConnection("jdbc:sqlite:mina.db");
          String query="select id , name , (1*"+tf2.getText()+") as quantity_n , cost , (cost*"+tf2.getText()+") as total from product where id ='"+tf1.getText()+"'"; 
          k=c.createStatement();
          rs=k.executeQuery(query); 
          
          while(rs.next()){
       z.add(new product(rs.getString("id"),rs.getString("name"),rs.getString("quantity_n"),rs.getString("cost"),rs.getString("total")));
...
//            area.setText(area.getText()+"----------------------------------------------------------------------------\n");
                ...
                try {
              c= DriverManager.getConnection("jdbc:sqlite:mina.db");    
              String query="insert into purchases_history values('"+bill+"',"+d+","+in+","+out+",'"+now.format(f)+"','"+now.format(f1)+"')";
              k=c.createStatement();
              rs=k.executeQuery(query);                    
                }
                catch (Exception ex) {
