...
public class purchases_returns extends JPanel{
    Connection c;
    Statement k;
    ResultSet rs;
...
/*****************************label******************************************/
    ...
    JLabel l3=new JLabel("مرتجعات الشراء");
...
public  purchases_returns() { 
this.setName("مرتجعات الشراء");
this.setLayout(null);
Color col=new Color(177,234,155);
...
//            area.setText(area.getText()+"----------------------------------------------------------------------------\n");
                ...
                try {
              c= DriverManager.getConnection("jdbc:sqlite:mina.db");    
              String query="insert into purchases_return_history values('"+bill+"',"+d+","+in+","+out+",'"+now.format(f)+"','"+now.format(f1)+"')";
              k=c.createStatement();
              rs=k.executeQuery(query);                    
                }
                catch (Exception ex) {
